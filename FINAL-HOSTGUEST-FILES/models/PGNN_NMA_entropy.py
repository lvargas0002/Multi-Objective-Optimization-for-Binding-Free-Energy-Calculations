...
def root_mean_squared_error(y_true, y_pred):
    return tf.sqrt(tf.reduce_mean(tf.square(y_pred - y_true))) #added sept 14
    #return K.sqrt(K.mean(K.square(y_pred - y_true))) commented sept 14th
def get_trained_model(X, y, shapes, batch_size, epochs=25):
    model = GBGraphConvModel(batch_size)
    model.compile(optimizer="rmsprop", loss=root_mean_squared_error)
    # Modify the learning rate in the model's optimizer
    #ADDED Sep14th 2024
    optimizer = model.optimizer
    optimizer.learning_rate.assign(0.001)
    #K.set_value(model.optimizer.learning_rate, 0.001) commented sept 14th
    model.input_shapes = shapes
    # Check the shape of y_train
    print(f"Original y_train shape: {y.shape}")

    # Adjust y_train based on actual shape
    if len(y.shape) == 1:
        # If y is a 1D array, reshape it to (batch_size, 1) or leave it as is
        y_train = np.array(y).reshape(batch_size, 1)  # Adjust 1 if multiple outputs expected
    else:
        # If y has more than 1 dimension, ensure it has the correct shape
        y_train = np.array(y)

    print(f"Reshaped y_train shape: {y_train.shape}")
    # Train the model
    hist = model.fit(X, y_train, epochs=epochs)
