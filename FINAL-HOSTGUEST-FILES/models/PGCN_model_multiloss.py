...
#                                              kernel_constraint=constraints.MaxNorm(maxnorm))
        self.dense7 = tf.keras.layers.Dense(1, name='dense7',
                                     kernel_initializer=tf.keras.initializers.Constant([0.3, 1, -1, -1]),
                                     bias_initializer=tf.keras.initializers.Zeros(),
                                     kernel_regularizer=regularizers.l2(l2), 
                                     bias_regularizer=regularizers.l2(l2), 
                                     kernel_constraint=constraints.MaxNorm(maxnorm))
...
def physical_consistency_loss(y_true,y_pred,physics_info):
    ...
    host = tf.gather(physics_info, [1], axis=1)  
    guest = tf.gather(physics_info, [2], axis=1) 
    complex_ = tf.gather(physics_info, [0], axis=1)  
